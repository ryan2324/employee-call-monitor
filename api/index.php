...
function hasColumn(PDO $p,string $table,string $column): bool {
    ...
    $q=$p->prepare('SELECT COUNT(*) FROM information_schema.columns WHERE table_schema=DATABASE() AND table_name=:t AND column_name=:c');
    $q->execute(['t'=>$table,'c'=>$column]); return $cache[$key]=((int)$q->fetchColumn()>0);
}
function exprColumn(PDO $p,string $table,string $column,string $alias,string $fallback='0'): string {
    return hasColumn($p,$table,$column) ? 'COALESCE(s.`'.$column.'`,' . $fallback . ') '.$alias : $fallback.' '.$alias;
}
...
function statusFor(array $d):string{
    ...
    $seen=strtotime((string)$d['last_seen']); if($seen===false || time()-$seen>(int)$config['app']['heartbeat_timeout_seconds']) return 'OFFLINE';
    if(($d['call_state']??'READY')==='IN_CALL') return 'IN_CALL';
    $changed=!empty($d['state_changed_at'])?strtotime((string)$d['state_changed_at']):$seen;
    return ($changed!==false && time()-$changed >= (int)$config['app']['idle_threshold_seconds'])?'IDLE':'READY';
}
...
function updateDailyStats(PDO $p,int $employeeId,string $date,int $duration,string $result):void{
    ...
    $vals=[':e',':d','1',':dur'];
    $updates=['calls_today=calls_today+1','talk_seconds_today=talk_seconds_today+VALUES(talk_seconds_today)'];
    if(hasColumn($p,'daily_employee_stats','successful_calls_today')){ $cols[]='successful_calls_today';$vals[]=':s';$updates[]='successful_calls_today=successful_calls_today+VALUES(successful_calls_today)'; }
    if(hasColumn($p,'daily_employee_stats','unsuccessful_calls_today')){ $cols[]='unsuccessful_calls_today';$vals[]=':f';$updates[]='unsuccessful_calls_today=unsuccessful_calls_today+VALUES(unsuccessful_calls_today)'; }
    $sql='INSERT INTO daily_employee_stats('.implode(',',$cols).') VALUES('.implode(',',$vals).') ON DUPLICATE KEY UPDATE '.implode(',',$updates);
...
try {
    ...
    if(($path==='/api' || $path==='/api/' || $path==='/api/index.php' || $path==='/api/health') && $method==='GET'){
        try{$p->query('SELECT 1');$tables=[];foreach(['managers','employees','devices','call_history','daily_employee_stats'] as $t){$q=$p->query("SHOW TABLES LIKE '$t'");$tables[$t]=(bool)$q->fetchColumn();}
    ...
    if($path==='/api/join/token' && $method==='POST'){
        ...
        $q=$p->prepare('INSERT INTO join_tokens(token,expires_at,created_by) VALUES(:t,FROM_UNIXTIME(:e),:m)');$q->execute(['t'=>$token,'e'=>$exp,'m'=>$_SESSION['manager_id']]);audit('CREATE_JOIN_TOKEN');
    ...
    if($path==='/api/employee/join' && $method==='POST'){
        ...
        try{
            ...
            else{$q=$p->prepare('INSERT INTO employees(name) VALUES(:n)');$q->execute(['n'=>$name]);$eid=(int)$p->lastInsertId();$q=$p->prepare("INSERT INTO devices(employee_id,device_id,model,last_seen,call_state,state_changed_at) VALUES(:e,:d,:m,NOW(),'READY',NOW())");$q->execute(['e'=>$eid,'d'=>$device,'m'=>$model?:null]);}
    ...
    if($path==='/api/employee/calls/sync' && $method==='POST'){
        ...
        $insert=$p->prepare("INSERT INTO call_history(employee_id,device_id,android_call_log_id,direction,contact_number,result,started_at,ended_at,duration_seconds)
            ...
            ON DUPLICATE KEY UPDATE contact_number=VALUES(contact_number),result=VALUES(result),ended_at=VALUES(ended_at),duration_seconds=VALUES(duration_seconds)");
        $added=0;$updated=0;
        foreach(array_slice($calls,0,100) as $c){
            ...
            $insert->execute(['e'=>$eid,'d'=>$device,'cid'=>$cid,'dir'=>$dir,'num'=>normalizeNumber($c['contact_number']??null),'result'=>$result,'start'=>$start,'end'=>$end,'dur'=>$dur]);
            $affected=$insert->rowCount();if($affected===1){$added++;}elseif($affected===2){$updated++;}
            if($affected===1){updateDailyStats($p,$eid,substr($start,0,10),$dur,$result);}
        }
    ...
    if($path==='/api/diagnostic' && $method==='GET'){
        ...
        try{$p->query('SELECT 1');$checks[]=['name'=>'Database connection','ok'=>true,'detail'=>'MySQL connection is working.'];}catch(Throwable $e){$errors[]=$e->getMessage();$checks[]=['name'=>'Database connection','ok'=>false,'detail'=>'Cannot connect to MySQL.'];}
    ...
    if($isHealth) out(['ok'=>false,'error'=>'Database connection failed','detail'=>'The API is reachable, but PHP could not connect to MySQL. Check the DB_* environment variables and database status.','php'=>PHP_VERSION],500);
