...
try {
    ...
    if($path==='/api/employee/command-ack' && $method==='POST'){     $b=body();      $commandId=(int)($b['command_id']??0);     $deviceId=trim((string)($b['device_id']??''));      if(!$commandId || !$deviceId){         out([             'ok'=>false,             'error'=>'Command ID and device ID are required'         ],422);     }      /*      * Acknowledge the warning.      */     $q=$p->prepare("         UPDATE warning_commands         SET acknowledged_at=NOW()         WHERE id=:i           AND device_id=:d           AND acknowledged_at IS NULL         RETURNING employee_id,command_type     ");      $q->execute([         'i'=>$commandId,         'd'=>$deviceId     ]);      $command=$q->fetch();      if(!$command){         out([             'ok'=>false,             'error'=>'Warning was already acknowledged or was not found'         ],404);     }      /*      * IMPORTANT:      *      * For an automatic idle warning, acknowledging it starts      * a completely NEW idle-warning cycle.      *      * state_changed_at = NOW()      *      * Therefore, with a 2-minute setting:      *      * Acknowledge at 10:05      *       ↓      * Timer starts at 10:05      *       ↓      * Next warning allowed at 10:07      */     if($command['command_type']==='AUTO_IDLE_WARNING'){         $q=$p->prepare("             UPDATE devices             SET state_changed_at=NOW()             WHERE device_id=:d               AND employee_id=:e               AND active=1               AND call_state='READY'         ");          $q->execute([             'd'=>$deviceId,             'e'=>(int)$command['employee_id']         ]);     }      out([         'ok'=>true,         'acknowledged'=>true,         'timer_reset'=>($command['command_type']==='AUTO_IDLE_WARNING')     ]); }
