...
try {
    ...
    if($path==='/api/diagnostic' && $method==='GET'){
        ...
        if($device!==''){
            ...
            else{$checks[]=['name'=>'Device registration','ok'=>true,'detail'=>'Registered to '.$r['name'].' (employee #'.$r['employee_id'].').'];$checks[]=['name'=>'Device active','ok'=>(bool)$r['active'],'detail'=>$r['active']?'Active':'Inactive'];$age=$r['last_seen']?time()-strtotime($r['last_seen'].' UTC'):null;$checks[]=['name'=>'Heartbeat','ok'=>$age!==null && $age<=90,'detail'=>$r['last_seen']?'Last seen '.$r['last_seen'].' ('.max(0,(int)$age).'s ago)':'No heartbeat recorded'];$checks[]=['name'=>'Call state','ok'=>true,'detail'=>$r['call_state']];}
        }
